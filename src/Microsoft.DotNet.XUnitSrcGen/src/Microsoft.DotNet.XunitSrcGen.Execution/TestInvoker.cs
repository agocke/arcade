...
namespace Microsoft.DotNet.XunitSrcGen
{
        ...
        where TTestCase : ITestCase
    {
        ...
        /// </summary>
        protected GeneratedTestClass TestClass { get; }
            ...
            if (!TestMethod.IsStatic && !Aggregator.HasExceptions)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            Timer.Aggregate(delegate
                            {
                                testClass = TestClass.Create(ConstructorArguments);
        ...
        public Task<decimal> RunAsync()
        {
            return Aggregator.RunAsync(async () =>
            {
                if (!CancellationTokenSource.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        var asyncLifetime = testClassInstance as Xunit.IAsyncLifetime;
