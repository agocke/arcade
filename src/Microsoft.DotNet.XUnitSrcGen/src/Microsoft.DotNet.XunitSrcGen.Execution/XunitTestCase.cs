    ...
    public abstract class XunitTestCase : TestMethodTestCase, IXunitTestCase
    {
      ...
      //  [Obsolete("Called by the de-serializer; should only be called by deriving classes for de-serialization purposes")]
        ...
        static IEnumerable<IAttributeInfo> GetCachedTraitAttributes(IAssemblyInfo assembly)
            => assemblyTraitAttributeCache.GetOrAdd(assembly.Name, (s) => assembly.GetCustomAttributes(typeof(ITraitAttribute)));

        static IEnumerable<IAttributeInfo> GetCachedTraitAttributes(ITypeInfo type)
            => typeTraitAttributeCache.GetOrAdd(type.Name, (s) => type.GetCustomAttributes(typeof(ITraitAttribute)));
